...
const SP = {
    matrinxa: {
        ...
        densityPerM2: 1.9,           // ~19.000 peixes/ha → 1,9/m²
        idealDensityPerHa: 19000,   // referência campo: 18.000-20.000 peixes/ha c/ aeração
        maxDensityPerHa: 20000,     // limite superior
        idealHarvestKg: 1.5,        // peso ideal despesca (FCR ótimo acima converte lento)
        kgPerM2Harvest: 2.85,       // biomassa/m² na despesca (19000×1.5/10000)
        kgPerM3: 3.5,
        minDepthM: 1.5,
        idealDepthM: 2.0,
        maxDepthM: 3.0,
    ...
    tambaqui: {
        ...
        densityPerM2: 0.7,          // 7.000 peixes/ha semi-intensivo c/ aeração
        idealDensityPerHa: 7000,    // referência de campo confirmada
        maxDensityPerHa: 8000,      // limite com boa aeração
        idealHarvestKg: 3.5,        // peso ideal de despesca (FCR ótimo, acima converte lento)
        kgPerM2Harvest: 2.45,       // biomassa/m² na despesca (7000×3.5/10000)
        kgPerM3: 2.5,               // densidade volumétrica estimada
...
.page-content{padding-bottom:calc(68px + env(safe-area-inset-bottom,0px));padding-top:8px;padding-left:14px;padding-right:14px;}
...
@supports(padding-top:env(safe-area-inset-top)){
  ...
  .page-content{padding-bottom:calc(68px + env(safe-area-inset-bottom))!important;padding-left:14px;padding-right:14px;}
}
...
function App() {
    ...
    const [syncing, setSyncing] = (0, useState)(false);
    const [lastSync, setLastSync] = (0, useState)(null);
    ...
    return (React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 14 } },
        ...
        section === "racao" && (React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 12 } },
            ...
            React.createElement("div", { className: "card", style: { padding: 16 } },
                React.createElement("div", { style: { fontSize: 13, fontWeight: 700, marginBottom: 10 } }, "\uD83D\uDCCF Granulometria por % de Prote\u00EDna (extrusada flutuante)"),
                React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 4 } }, [
                    { p: "45% / 40%", mm: "1,5 \u00B7 1,8 \u00B7 2 \u00B7 3 mm" },
                    { p: "36%", mm: "2 \u00B7 5 mm" },
                    { p: "32%", mm: "4 \u00B7 6 mm" },
                    { p: "28%", mm: "4 \u00B7 6 \u00B7 8 \u00B7 10 \u00B7 12 mm" },
                ].map(r => React.createElement("div", { key: r.p, style: { display: "flex", justifyContent: "space-between", fontSize: 12, padding: "7px 0", borderBottom: "1px solid var(--border)" } },
                    React.createElement("span", { style: { color: "var(--muted)" } }, r.p),
                    React.createElement("span", { style: { fontFamily: "var(--mono)", fontWeight: 700 } }, r.mm))))),
        ...
        React.createElement("div", { style: { marginBottom: 20 } },
            ...
            ...(tank.targetWeightKg > 0 ? [{
                ...
                val: (function(){
                    ...
                    var aHa = (tank.areaM2||0) / 10000;
                    var baseCount = Math.floor((sp3.idealDensityPerHa || sp3.densityPerM2*10000) * aHa);
                    if (tank.targetWeightKg && sp3.idealHarvestKg && tank.targetWeightKg !== sp3.idealHarvestKg) {
                      var biomCap = (sp3.kgPerM2Harvest || sp3.densityPerM2 * tank.targetWeightKg) * (tank.areaM2||0);
                      baseCount = Math.floor(biomCap / tank.targetWeightKg);
                    }
                    return baseCount.toLocaleString("pt-BR");
                })(),
                color: "#a855f7"
            }] : []),
...
function DailyTab({ tank, phase, dailyFeedKg, sp, session, role }) {
    ...
    function handleSave() {
        ...
        // Auto-update tank avg weight if biometry entered during engineer visit
        if (engVisit.active && engVisit.bioWeightG && parseFloat(engVisit.bioWeightG) > 0) {
            var newW = parseFloat(engVisit.bioWeightG);
            var bioH = [...(tank.bioHistory || []), {
                date: activeDate,
                avgWeightG: newW,
                avgLengthCm: 0,
                fishCount: tank.fishCount || 0,
                source: "visita_engenheiro",
                _by: session ? session.name : "",
                _at: new Date().toLocaleString("pt-BR")
            }];
    ...
    return (React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 14 } },
        ...
        engVisit.active && (React.createElement("div", { className: "card", style: { padding: 18, border: "1px solid rgba(167,139,250,0.3)", background: "rgba(167,139,250,0.04)" } },
            ...
            React.createElement("div", { className: "grid2" },
                ...
                React.createElement("div", null,
                    React.createElement("lbl", { style: { color: "#a78bfa" } }, "\u2696\uFE0F Peso M\u00E9dio Biometria (g)"),
                    React.createElement("input", { type: "number", step: "1", min: "1",
                        className: "inp", placeholder: "ex: 850",
                        value: engVisit.bioWeightG || "",
                        onChange: function(e) { setEngVisit(function(p) { return Object.assign({}, p, { bioWeightG: e.target.value }); }); }
                    }),
                    engVisit.bioWeightG && parseFloat(engVisit.bioWeightG) > 0
                      ? React.createElement("div", { style:{fontSize:11,color:"#a78bfa",marginTop:4,fontWeight:600} },
                          "\u2192 " + ((function(){ var ph = typeof getPhase !== 'undefined' ? getPhase(tank.species, parseFloat(engVisit.bioWeightG)) : null; return ph ? ph.name : ""; })()) + " \u2022 Atualiza o tanque automaticamente ao salvar")
                      : null
                ),                React.createElement("div", null,
            ...
            React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 13 } },
                ...
                React.createElement("div", { style:{display:"flex",alignItems:"center",gap:10,padding:"8px 0"} },
                    ...
                    form.targetWeightKg && parseFloat(form.targetWeightKg) > 0 && (function(){
                        ...
                        // Use field-validated density: idealDensityPerHa or densityPerM2
                        var areaHa = areaM2 / 10000;
                        var maxByDensity = areaM2 > 0
                          ? Math.floor((sp2.idealDensityPerHa || sp2.densityPerM2 * 10000) * areaHa)
                          : 0;
                        // Adjust if target weight differs from ideal harvest weight
                        if (targetKg && sp2.idealHarvestKg && targetKg !== sp2.idealHarvestKg) {
                          // At different target: biomass capacity stays same, fish count changes
                          var biomassCapacity = (sp2.kgPerM2Harvest || sp2.densityPerM2 * targetKg) * areaM2;
                          maxByDensity = Math.floor(biomassCapacity / targetKg);
                        }
...
function FinanceiroModal({ onClose }) {
  ...
  var swipeRef = useSwipeToClose(onClose);
    ...
    const totalExpTanks = Object.values(useApp().expenses || {}).flat().reduce